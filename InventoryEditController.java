...
public class InventoryEditController {
	...
	public void btnSaveOnAction(ActionEvent event) {
		if(checkData()) {
			if(ItemDBControl.UpdateItem(Item_ID,txfItemCode.getText(), txfItemName.getText(), txaDescription.getText(),
				...
				cmbUnit.getSelectionModel().getSelectedIndex()+1)){
	...
	public void btnCancelOnAction(ActionEvent event) {
		...
		alert.showAndWait().ifPresent(type -> {
		        if ( type == okButton) {
		        	...
		        	txfPricePurchased.clear();
		        	txfPriceSelling.clear();
		        	txfPriceCustomer.clear();
		        	txfCustomizeQuantity.clear();
		        	
		    		Item_ID=InventoryViewController.getSelected().getItem_ID();
		        	selected= InventoryViewDBControl.getItemData(Item_ID);
		    		txfItemCode.setText(selected.getItem_Code());
		    		txfItemName.setText(selected.getItem_Name());
		    		txaDescription.setText(selected.getDescription());
		    		txfPricePurchased.setText(selected.getPrice_Purchased()+"");
		    		txfPriceSelling.setText(selected.getPrice_Selling()+"");
		    		txfPriceCustomer.setText(selected.getPrice_Customer()+"");
		    		spinnerQuantity.getValueFactory().setValue(selected.getQuantity());
		    		txfCustomizeQuantity.setText(selected.getCustomize_Quantity());
		    		spinnerDefaultQuantity.getValueFactory().setValue(selected.getDefault_Quantity());
		    		spinnerMinimum.getValueFactory().setValue(selected.getMinimum());
		    		cmbBrand.getSelectionModel().select(selected.getBrand_Name());
		    		cmbUnit.getSelectionModel().select(selected.getUnit_Name());
		    		Item_ID=selected.getItem_ID();
		        }
		});
	}
	...
	void initialize() {
		...
		spinnerDefaultQuantity.getValueFactory().setValue(selected.getDefault_Quantity());
		spinnerMinimum.getValueFactory().setValue(selected.getMinimum());
		cmbBrand.getSelectionModel().select(Integer.parseInt(selected.getBrand_Name())-1);
		cmbUnit.getSelectionModel().select(Integer.parseInt(selected.getUnit_Name())-1);
		 	}
